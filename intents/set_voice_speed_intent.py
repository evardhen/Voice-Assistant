...
class CustomSetVoiceSpeedTool(BaseTool):
    ...
    description = "useful when you want to change the voice speed of the assistant Luna. The voice speed is between 150 and 200. In some languages, like the german language, a float is serarated by a comma. Python allows this separation only with a \".\". Please change the volume to that format."
...
def set_voice_speed(voice_speed):
    ...
    if voice_speed >= 150 and voice_speed <= 200:
        ...
        return f"Could not change the volume. The volume has to be between 150 and 200, but you entered {voice_speed}"
    else:
